import { loadReferenceRegistry } from "./qcae-api.js";
...
try {
  const registry = await loadReferenceRegistry();
  ...
  for (const [locale, mapping] of Object.entries(registry.activeVariants)) {
    target.append(h("section", {},
      h("h2", {}, languageNames[locale] ?? locale),
      h("ul", { class: "reference-list" }, mapping.referenceIds.map((id) => referenceCard(byId.get(id))))
    ));
  }
  target.append(h("section", {},
    h("h2", {}, "Additional versions identified"),
    h("p", {}, "These versions remain disabled until their exact validated wording is available."),
    h("div", { class: "candidate-grid" }, registry.integrationCandidates.map((candidate) =>
      h("article", { class: "candidate-card" },
        h("h3", {}, candidate.language),
        h("p", {}, h("span", { class: "status-badge" }, candidate.status)),
        h("ul", { class: "reference-list" }, candidate.referenceIds.map((id) => referenceCard(byId.get(id))))
      )
    ))
  ));
} catch (error) {
